-- Fix ambiguous column reference in get_events_with_details function
-- The issue is that extra_participants_count appears in multiple contexts

-- Drop the existing function first
DROP FUNCTION IF EXISTS get_events_with_details(uuid);

-- Recreate get_events_with_details function with fully qualified column references
...
BEGIN
    ...
    FROM events e
    LEFT JOIN (
        SELECT ep1.event_id, COUNT(*) as count
        FROM event_participants ep1
        GROUP BY ep1.event_id
    ) ep_count ON e.id = ep_count.event_id
    LEFT JOIN (
        SELECT ep2.event_id, SUM(ep2.extra_participants_count) as total_extras
        FROM event_participants ep2
        GROUP BY ep2.event_id
    ) ep_extras ON e.id = ep_extras.event_id
    LEFT JOIN (
        SELECT ep3.event_id, TRUE as joined
        FROM event_participants ep3
        WHERE ep3.user_id = user_id_param
