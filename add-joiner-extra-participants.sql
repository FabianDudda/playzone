-- Add extra participants support for event joiners
...
-- Step 3: Update get_events_with_details function to include all extra participants
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(ep_count.count, 0) + e.extra_participants_count + COALESCE(ep_extras.total_extras, 0) as participant_count,
...
-- Step 4: Update can_join_event function to account for all extra participants
...
BEGIN
    ...
    total_participants := participant_count + event_record.extra_participants_count + joiner_extras;
